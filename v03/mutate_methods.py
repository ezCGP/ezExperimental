...
def mutate_single_input(indiv: IndividualMaterial, block_index: int, indiv_def: IndividualDefinition):
	'''
	...
	block_def = indiv_df[block_index]
	...
	for node_index in choices:
		if node_index < block_def.main_count:
			...
			req_dtype = block_def.get_node_dtype(node_index, "inputs")[ith_input]
			new_input = block_def.get_random_input(dtype=req_dtype, max_=node_index, exclude=[current_input_index])
			...
			new_output_index = block_def.get_random_input(dtype=req_dtype, min_=0, exclude=[current_output_index])
			if new_output_index is None:
				# failed to find new node
				continue
			else:
				self[node_index] = new_output_index
				break # active_node finally mutated


def mutate_single_ftn(indiv: IndividualMaterial, block_index: int, indiv_def: IndividualDefinition):
	'''
	...
	for node_index in choices:
		...
		req_output_dtype = block_def.operator_dict[new_ftn]["output"]
		...
		for input_index in block[node_index]["inputs"]:
			exist_input_dtype = block_def.get_node_dtype(input_index, "output") #instead of verify from current node, goes to input
		...
		for ith_input, (new_input, input_dtype) in enumerate(zip(new_inputs, req_input_dtypes)):
			if new_input is None:
				new_inputs[ith_input] = block_def.get_random_input(req_dtype=input_dtype, max_=node_index)
		...
		exist_arg_dtypes = block_def.get_node_dtype(node_index, "args")
...
def mutate_single_argindex(indiv: IndividualMaterial, block_index: int, indiv_def: IndividualDefinition):
	'''
	words
	'''
	block = indiv[block_index]
	...
	if len(block.active_args) > 0:
		...
		for node_index in choices:
			...
			arg_dtype = block_def.get_node_dtype(node_index, "args")[ith_arg]
...
def mutate_single_argvalue(indiv: IndividualMaterial, block_index: int, indiv_def: IndividualDefinition):
	'''
	words
	'''
	block = indiv[block_index]
